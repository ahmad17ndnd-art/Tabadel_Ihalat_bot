...
def init_db():
    ...
    # جدول أسعار فئات الإعلانات (حد أدنى / أقصى يحدده الأدمن)
    c.execute("""
    ...
    # جدول الإعلانات (لوحة تبادل الإعلانات)
    c.execute("""
...
def get_category_price(category: str):
    ...
    conn.close()
    if not row:
        return DEFAULT_CATEGORY_PRICES.get(category, (100, 500))
    return row[0], row[1]
...
def create_ad(owner_id, owner_username, category, link, description, post_price, reward_points, verify_mode, channel_id):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute(
        """INSERT INTO ads (owner_id, owner_username, category, link, description, post_price,
                             reward_points, verify_mode, channel_id, status, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)""",
        (owner_id, owner_username, category, link, description, post_price, reward_points,
...
def get_ads_by_status(status, category=None):
    ...
    if category:
        c.execute(
            "SELECT ad_id, owner_id, owner_username, category, link, description, post_price, reward_points, verify_mode, channel_id, status, created_at "
            "FROM ads WHERE status = ? AND category = ? ORDER BY created_at DESC",
            (status, category)
        )
    else:
        c.execute(
...
def get_ads_by_owner(owner_id):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT ad_id, category, link, post_price, reward_points, status FROM ads WHERE owner_id = ? ORDER BY created_at DESC",
        (owner_id,)
    )
    rows = c.fetchall()
    conn.close()
...
def open_ad_for_user(ad_id, user_id):
    """يفتح إعلان لمستخدم (أول مرة أو بعد الرفض) ويرجّع الحالة الحالية."""
...
def get_latest_pending_proof_request(user_id):
    """آخر إعلان (يدوي) بانتظار سكرين شوت من هذا المستخدم."""
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute(
...
async def admin_navigation_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_cat_prices":
        ...
        await query.message.reply_text(
            "💵 أسعار فئات الإعلانات الحالية (اضغط لتعديل فئة):",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return
    ...
    if data.startswith("adlist:"):
        ...
        for ad in ads:
            ...
            text = (
                ...
                f"💵 سعر النشر المدفوع: {ad['post_price']} نقطة\n"
                f"🎁 مكافأة التنفيذ: {ad['reward_points']} نقطة\n"
...
async def ask_category_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mn, mx = get_category_price(cat)
    await query.message.reply_text(
        f"💵 السعر الحالي لفئة {CATEGORY_LABELS.get(cat, cat)}: {mn} - {mx}\n"
...
async def save_category_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) != 2:
        await update.message.reply_text("❌ الصيغة غير صحيحة. أرسل رقمين مفصولين بمسافة، مثال: 800 1500")
        return ConversationHandler.END
    try:
...
async def ask_ad_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for cat in CATEGORY_ORDER:
        mn, mx = get_category_price(cat)
        keyboard.append([InlineKeyboardButton(
            f"{CATEGORY_LABELS[cat]} ({mn}-{mx} نقطة)", callback_data=f"adcat:{cat}"
        )])
    await query.message.reply_text(
...
async def ad_category_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mn, mx = get_category_price(cat)
    context.user_data["new_ad"]["price_min"] = mn
    context.user_data["new_ad"]["price_max"] = mx
    await query.message.reply_text(
...
async def ad_got_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["new_ad"]["link"] = update.message.text.strip()
    await update.message.reply_text("📝 الآن أرسل وصف/نص الإعلان (شو المطلوب من الشخص ينفذه؟):")
...
async def ad_got_desc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🎁 كم نقطة بدك تعطي كمكافأة لكل شخص ينفذ الإعلان (اشتراك/تفاعل)؟\nأرسل رقم:"
    )
...
async def ad_got_reward(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mn = context.user_data["new_ad"]["price_min"]
    mx = context.user_data["new_ad"]["price_max"]
    await update.message.reply_text(
        f"💵 الآن أرسل سعر نشر الإعلان (يُخصم من رصيدك مرة واحدة).\n"
        f"يجب أن يكون بين {mn} و {mx} نقطة:"
    )
    return WAITING_AD_PRICE_INPUT


async def ad_got_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    new_ad = context.user_data.get("new_ad", {})
    mn, mx = new_ad.get("price_min"), new_ad.get("price_max")
    try:
    ...
    if price < mn or price > mx:
        await update.message.reply_text(f"❌ السعر يجب أن يكون بين {mn} و {mx} نقطة. حاول مجدداً:")
        return WAITING_AD_PRICE_INPUT

    context.user_data["new_ad"]["post_price"] = price
    cat = new_ad["category"]
...
async def finalize_ad_creation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await telegram_app.bot.send_message(
            ...
            f"🔗 الرابط: {ad['link']}\n"
            f"📝 الوصف: {ad['description']}\n"
            f"💵 سعر النشر: {ad['post_price']} نقطة\n"
            f"🎁 مكافأة التنفيذ: {ad['reward_points']} نقطة\n"
            f"🔍 طريقة التحقق: {verify_txt}",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
...
async def show_my_ads(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for ad_id, category, link, price, reward, status in ads:
        text = (
            f"📌 إعلان #{ad_id} — {CATEGORY_LABELS.get(category, category)}\n"
            f"🔗 الرابط: {link}\n"
            f"💵 سعر النشر: {price} نقطة | 🎁 المكافأة: {reward} نقطة\n"
...
async def show_earn_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for ad in ads:
        ...
        if status in ("approved", "sent", "proof_submitted", "review_requested"):
            continue
        keyboard = [[InlineKeyboardButton("🎯 فتح الإعلان", callback_data=f"earnad:{ad['ad_id']}")]]
        await query.message.reply_text(
            f"📌 إعلان #{ad['ad_id']} — {CATEGORY_LABELS.get(cat, cat)}\n"
            f"📝 {ad['description']}\n"
...
async def show_earn_ad_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("🔗 فتح الرابط", url=ad["link"])],
        [InlineKeyboardButton("✅ نفذت المطلوب", callback_data=f"earnconfirm:{ad_id}")]
    ]
    verify_hint = "بعد التنفيذ اضغط الزر تحت وراح نتحقق تلقائياً." if ad["verify_mode"] == "auto" \
        else "بعد التنفيذ اضغط الزر تحت، وبعدين ابعت سكرين شوت (صورة) كإثبات."
    await query.message.reply_text(
        f"📌 إعلان #{ad_id} — {CATEGORY_LABELS.get(ad['category'], ad['category'])}\n"
        f"📝 {ad['description']}\n🎁 المكافأة: {ad['reward_points']} نقطة\n\n{verify_hint}",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def earn_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row and row[0] == "approved":
        ...
        return
    if row and row[0] in ("proof_submitted", "review_requested"):
        await query.message.reply_text("🧾 إثباتك قيد المراجعة من صاحب الإعلان حالياً.")
        return
    ...
    # وضع الإثبات اليدوي
    await query.message.reply_text("🧾 تمام! هلق ابعت سكرين شوت (صورة) يثبت إنك نفذت المطلوب.")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1) رد على طلب مراجعة من صاحب إعلان
    ...
    if review_ad_id:
        ...
        try:
            keyboard = [
                [InlineKeyboardButton("✅ دفع النقاط", callback_data=f"adpay:{review_ad_id}:{user.id}"),
                 InlineKeyboardButton("❌ رفض", callback_data=f"adrejc:{review_ad_id}:{user.id}")],
                [InlineKeyboardButton("🔍 مراجعة", callback_data=f"adrev:{review_ad_id}:{user.id}")]
            ]
            await telegram_app.bot.send_photo(
                ad["owner_id"], photo,
                caption=f"🧾 رد جديد على المراجعة — إعلان #{review_ad_id}\nمن: {user.username or user.full_name} (ID: {user.id})",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
        except Exception as e:
            logger.error(f"Failed to forward review reply: {e}")
        await update.message.reply_text("✔ تم إرسال ردك لصاحب الإعلان، بانتظار الموافقة.")
        return

    # 2) إثبات صورة لإعلان بانتظار سكرين شوت
    ...
    if pending_ad_id:
        ...
        try:
            keyboard = [
                [InlineKeyboardButton("✅ دفع النقاط", callback_data=f"adpay:{pending_ad_id}:{user.id}"),
                 InlineKeyboardButton("❌ رفض", callback_data=f"adrejc:{pending_ad_id}:{user.id}")],
                [InlineKeyboardButton("🔍 مراجعة", callback_data=f"adrev:{pending_ad_id}:{user.id}")]
            ]
            await telegram_app.bot.send_photo(
                ad["owner_id"], photo,
                caption=(
                    f"🧾 إثبات جديد — إعلان #{pending_ad_id} ({CATEGORY_LABELS.get(ad['category'], ad['category'])})\n"
        ...
        await update.message.reply_text("✔ تم إرسال إثباتك لصاحب الإعلان، رح توصلك النقاط بعد الموافقة.")
        return
...
async def handle_text_or_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # رد على طلب مراجعة (نص بدل صورة)
    review_ad_id = get_latest_review_request(user.id)
    if review_ad_id:
        ad = get_ad(review_ad_id)
        set_completion_status(review_ad_id, user.id, "proof_submitted")
        try:
            keyboard = [
                [InlineKeyboardButton("✅ دفع النقاط", callback_data=f"adpay:{review_ad_id}:{user.id}"),
                 InlineKeyboardButton("❌ رفض", callback_data=f"adrejc:{review_ad_id}:{user.id}")],
            ...
            await telegram_app.bot.send_message(
                ad["owner_id"],
                f"🧾 رد نصي على المراجعة — إعلان #{review_ad_id}\n"
                f"من: {user.username or user.full_name} (ID: {user.id})\n\n{text}",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
        except Exception as e:
            logger.error(f"Failed to forward review text reply: {e}")
        await update.message.reply_text("✔ تم إرسال ردك لصاحب الإعلان.")
        return
...
async def ad_owner_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("adpay:"):
        ...
        await query.message.reply_text("✔ تم دفع النقاط للمستخدم.")
        try:
            await telegram_app.bot.send_message(
                uid, f"🎉 تم قبول إثباتك لإعلان #{ad_id}!\n💰 حصلت على {ad['reward_points']} نقطة."
            )
        except Exception:
            pass
        return
...
async def ask_review_note(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text("🔍 اكتب الرسالة يلي بدك ترسلها لهذا الشخص (مثلاً وضّح شو ناقص بالإثبات):")
...
async def save_review_note(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await telegram_app.bot.send_message(
            uid,
            f"🔍 صاحب الإعلان #{ad_id} طلب منك مراجعة/توضيح:\n\n{note}\n\n"
            f"يرجى فتح البوت وإرسال ردك أو إثبات جديد (نص أو صورة)."
        )
        await update.message.reply_text("✔ تم إرسال طلب المراجعة، وبتوصلك رسالة فور ما يرد.")
    except Exception as e:
        await update.message.reply_text(f"خطأ أثناء الإرسال: {e}")
...
ad_create_conv = ConversationHandler(
    ...
    states={
        WAITING_AD_LINK: [MessageHandler(filters.TEXT & ~filters.COMMAND, ad_got_link)],
        ...
        WAITING_AD_PRICE_INPUT: [MessageHandler(filters.TEXT & ~filters.COMMAND, ad_got_price)],
...
telegram_app.add_handler(CallbackQueryHandler(ask_ad_category, pattern="^publish_ad$"))
