...
async def send_admin_home(message):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📊 إحصائيات آخر رسالة جماعية", callback_data="adm_broadcast_stats")],
...
async def broadcast_stats_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if not config.is_admin(update.effective_user.id):
        return
    s = db.get_settings()
    total = s.get("last_broadcast_total") or 0
    if not total:
        await query.message.reply_text("📊 لسا ما أرسلت أي رسالة جماعية.")
        return
    sent = s.get("last_broadcast_sent") or 0
    failed = s.get("last_broadcast_failed") or 0
    at = s.get("last_broadcast_at") or "—"
    preview = s.get("last_broadcast_preview") or "—"
    text = (
        "📊 إحصائيات آخر رسالة جماعية\n\n"
        f"🕒 وقت الإرسال: {at}\n"
        f"📝 مقتطف من الرسالة: {preview}\n\n"
        f"👥 إجمالي المستهدفين: {fmt(total)}\n"
        f"✅ وصلتهم فعليًا: {fmt(sent)}\n"
        f"❌ ما وصلتهم (حظروا البوت / حسابهم متوقف): {fmt(failed)}\n\n"
        "⚠️ ملاحظة: تيليجرام ما بيعطي البوتات معرفة هل المستخدم قرأ الرسالة فعليًا، "
        "الأرقام هون بتعكس نجاح/فشل التوصيل بس."
    )
...
async def comp_menu_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("➕ إنشاء مسابقة جديدة", callback_data="adm_comp_new")]]
    if comp:
        status_line = f"📌 المسابقة الحالية: {comp['title']}"
        keyboard.append([InlineKeyboardButton("✏️ تعديل المسابقة الحالية", callback_data="adm_comp_edit")])
        keyboard.append([InlineKeyboardButton("🗑 حذف المسابقة الحالية", callback_data="adm_comp_delete_ask")])
    else:
        status_line = "📌 ما في مسابقة حالياً."
    keyboard.append([InlineKeyboardButton("✏️ تعديل عدد إحالات مستخدم", callback_data="adm_setref_user")])
    keyboard.append([InlineKeyboardButton("🔙 رجوع", callback_data="adm_home")])
    await query.message.edit_text(f"🏆 قائمة المسابقات\n{status_line}", reply_markup=InlineKeyboardMarkup(keyboard))


async def comp_edit_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if not config.is_admin(update.effective_user.id):
        return
    comp = db.get_active_competition()
    if not comp:
        await query.message.edit_text("❌ ما في مسابقة حالية لتعديلها.")
        return
    text = (
        f"🏆 {comp['title']}\n"
    ...
    keyboard = [
        [InlineKeyboardButton("✏️ العنوان", callback_data="adm_comp_editf:title")],
        [InlineKeyboardButton("✏️ الوصف", callback_data="adm_comp_editf:description")],
        [InlineKeyboardButton("✏️ جائزة المركز الأول", callback_data="adm_comp_editf:prize_1")],
        [InlineKeyboardButton("✏️ جائزة المركز الثاني", callback_data="adm_comp_editf:prize_2")],
        [InlineKeyboardButton("✏️ جائزة المركز الثالث", callback_data="adm_comp_editf:prize_3")],
        [InlineKeyboardButton("🔙 رجوع", callback_data="adm_comp_menu")],
    ]
...
COMP_FIELD_LABELS = {
    "title": "العنوان",
    "description": "الوصف",
    "prize_1": "جائزة المركز الأول",
    "prize_2": "جائزة المركز الثاني",
    "prize_3": "جائزة المركز الثالث",
}


async def comp_edit_field_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if not config.is_admin(update.effective_user.id):
        return
    ...
    if not comp:
        await query.message.edit_text("❌ ما في مسابقة حالية لتعديلها.")
        return
    context.user_data["comp_edit_id"] = comp["competition_id"]
    context.user_data["comp_edit_field"] = field
    context.user_data["awaiting"] = "admin:comp_edit_value"
    label = COMP_FIELD_LABELS[field]
    await query.message.reply_text(f"✏️ اكتب القيمة الجديدة لـ «{label}»:")


async def comp_delete_ask_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if not config.is_admin(update.effective_user.id):
        return
    comp = db.get_active_competition()
    if not comp:
        await query.message.edit_text("❌ ما في مسابقة حالية لحذفها.")
        return
    keyboard = [
        [InlineKeyboardButton("✅ نعم، احذفها", callback_data=f"adm_comp_delete_yes:{comp['competition_id']}")],
        [InlineKeyboardButton("❌ إلغاء", callback_data="adm_comp_menu")],
    ]
    await query.message.edit_text(
        f"⚠️ متأكد بدك تحذف مسابقة «{comp['title']}»؟ رح تختفي فورًا من عند كل المستخدمين.",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )


async def comp_delete_yes_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if not config.is_admin(update.effective_user.id):
        return
    ...
    await query.message.edit_text("✔ تم حذف المسابقة.")
...
async def admin_text_router(update: Update, context: ContextTypes.DEFAULT_TYPE, awaiting: str):
    ...
    if kind == "price_min":
        ...
        total = len(user_ids)
        failed = total - sent
        db.record_broadcast_stats(total, sent, failed, text)
        context.user_data["awaiting"] = None
    ...
    elif kind == "comp_edit_value":
        comp_id = context.user_data.get("comp_edit_id")
        field = context.user_data.get("comp_edit_field")
        value = None if (field == "description" and text.strip() in ("-", "تجاوز", "الغاء", "إلغاء")) else text.strip()
        db.update_competition_field(comp_id, field, value)
        context.user_data["awaiting"] = None
        context.user_data.pop("comp_edit_id", None)
        context.user_data.pop("comp_edit_field", None)
        label = COMP_FIELD_LABELS.get(field, field)
        await update.message.reply_text(f"✔ تم تعديل «{label}» بنجاح.")
...
def register_handlers(telegram_app):
    ...
    telegram_app.add_handler(CallbackQueryHandler(broadcast_stats_click, pattern="^adm_broadcast_stats$"))
    ...
    telegram_app.add_handler(CallbackQueryHandler(comp_edit_click, pattern="^adm_comp_edit$"))
    telegram_app.add_handler(CallbackQueryHandler(comp_edit_field_click, pattern="^adm_comp_editf:"))
    telegram_app.add_handler(CallbackQueryHandler(comp_delete_ask_click, pattern="^adm_comp_delete_ask$"))
    telegram_app.add_handler(CallbackQueryHandler(comp_delete_yes_click, pattern="^adm_comp_delete_yes:"))
