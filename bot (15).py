...
# ==================== حماية من النشاط الغريب (تكرار سريع = كابتشا) ====================

FLOOD_WINDOW_SECONDS = 10
FLOOD_THRESHOLD = 8
CAPTCHA_VALID_MINUTES = 15

FRUIT_IMAGE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets", "fruits")
FRUIT_LABELS = {
    "apple": "🍎 تفاحة",
    "banana": "🍌 موزة",
    "grapes": "🍇 عنب",
...
FRUIT_EMOJIS = {k: v.split()[0] for k, v in FRUIT_LABELS.items()}
FRUIT_KEYS = list(FRUIT_LABELS.keys())


async def send_fruit_captcha(context: ContextTypes.DEFAULT_TYPE, chat_id: int, purpose: str):
    """
    كابتشا صورة موحّدة تُستخدم بكل مكان بالبوت محتاج تحقق (دخول أول مرة، نشاط مشبوه، محاولات فاشلة متكررة...).
    purpose: مفتاح يحدد أي تدفق ينتظر الجواب (مثلاً 'entry' / 'security' / 'cheque').
    لو الصورة المطلوبة (assets/fruits/<fruit>.png) غير موجودة، بترجع تلقائيًا لإيموجي كبير بدالها بدل ما تفشل.
    """
    correct = random.choice(FRUIT_KEYS)
    context.user_data[f"fruitcap_{purpose}_answer"] = correct
    options = FRUIT_KEYS.copy()
    random.shuffle(options)
    keyboard = [
        [InlineKeyboardButton(FRUIT_LABELS[f], callback_data=f"fruitcap:{purpose}:{f}") for f in options[0:3]],
        [InlineKeyboardButton(FRUIT_LABELS[f], callback_data=f"fruitcap:{purpose}:{f}") for f in options[3:6]],
    ]
    img_path = os.path.join(FRUIT_IMAGE_DIR, f"{correct}.png")
    if os.path.isfile(img_path):
        with open(img_path, "rb") as img:
            await context.bot.send_photo(
                chat_id, photo=img,
                caption="🔒 تحقق أمني — اضغط اسم الفاكهة الظاهرة بالصورة:",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
    else:
        logger.warning(f"fruit captcha image missing: {img_path} — falling back to emoji")
        big_emoji = FRUIT_EMOJIS[correct] * 5
        await context.bot.send_message(
            chat_id,
            f"🔒 تحقق أمني\n\n{big_emoji}\n\nاضغط اسم الفاكهة الظاهرة فوق:",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
...
async def fruit_captcha_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    _, purpose, chosen = query.data.split(":")
    expected = context.user_data.get(f"fruitcap_{purpose}_answer")
    ...
    if chosen != expected:
        try:
            ...
            pass
        await send_fruit_captcha(context, update.effective_user.id, purpose)
        return

    context.user_data.pop(f"fruitcap_{purpose}_answer", None)
    try:
        await query.message.delete()
    except Exception:
        pass
    ...
    if purpose == "entry":
        db.set_field("users", "user_id", update.effective_user.id, "verified", 1)
        pending = context.user_data.pop("pending_entry", {})
        await continue_start_flow(
            update, context,
            pending.get("is_new", False), pending.get("ref_by"),
            pending.get("admin_link_code"), pending.get("cheque_code")
        )
    elif purpose == "security":
        verified_until = context.application.bot_data.setdefault("captcha_verified_until", {})
        verified_until[update.effective_user.id] = datetime.now() + timedelta(minutes=CAPTCHA_VALID_MINUTES)
        await context.bot.send_message(update.effective_user.id, "✅ تم التحقق، تقدر تكمل عادي.")
    elif purpose == "cheque":
        code = context.user_data.pop("cheque_captcha_return_code", None)
        context.user_data["cheque_fail_count"] = 0
        context.user_data["awaiting"] = f"cheque_redeem_pass:{code}" if code else None
        await context.bot.send_message(
            update.effective_user.id,
            "✅ تم التحقق. تفضل أدخل كلمة المرور الصحيحة:" if code else "✅ تم التحقق."
        )


async def security_flood_gate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    يراقب عدد الحركات (رسائل/ضغطات) لكل مستخدم بفترة قصيرة. لو تجاوز حد معقول (نشاط غريب/بوت مؤتمت)
    يطلب كابتشا صورة (فاكهة) قبل ما يسمحله يكمل. غير مطبّق على الأدمن.
    """
    user = update.effective_user
    if not user or config.is_admin(user.id):
        return
    if context.user_data.get("fruitcap_security_answer"):
        return  # كابتشا معلّقة أصلاً بانتظار جوابها

    now = datetime.now()
    bot_data = context.application.bot_data
    verified_until = bot_data.setdefault("captcha_verified_until", {})
    if verified_until.get(user.id) and verified_until[user.id] > now:
        return  # تحقق قبل شوي، لسا ضمن فترة السماح

    activity = bot_data.setdefault("activity_log", {})
    timestamps = activity.setdefault(user.id, [])
    ...
    timestamps.clear()  # نصفّر العداد حتى ما يعيد نفس الكابتشا كل حركة لحد ما يحلها
    if update.callback_query:
        await update.callback_query.answer("🔒 في تحقق أمني مطلوب — شوف آخر رسالة بالمحادثة.", show_alert=True)
    await send_fruit_captcha(context, user.id, "security")
    raise ApplicationHandlerStop
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not config.is_admin(user.id):
        ...
        if u and not u.get("verified"):
            context.user_data["pending_entry"] = {
                "ref_by": ref_by, "admin_link_code": admin_link_code,
                "is_new": is_new, "cheque_code": cheque_code,
            }
            await send_fruit_captcha(context, user.id, "entry")
            return

    await continue_start_flow(update, context, is_new, ref_by, admin_link_code, cheque_code)


async def continue_start_flow(update: Update, context: ContextTypes.DEFAULT_TYPE, is_new, ref_by, admin_link_code, cheque_code):
    user = update.effective_user
...
async def finish_start(update: Update, context: ContextTypes.DEFAULT_TYPE, is_new, ref_by, admin_link_code):
    ...
    if is_new:
        settings = db.get_settings()
        gift = settings.get("welcome_gift_points", 0)
        if gift:
            db.add_balance(user.id, gift, kind="welcome_gift", note="هدية ترحيبية عند أول دخول")
    ...
    if is_new and gift:
        await update.effective_message.reply_text(f"🎁 هدية ترحيبية: حصلت على {fmt_lira(gift)} ليرة فوراً!")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.effective_user or not update.message:
        # صورة واردة من قناة (channel post) وليست من مستخدم — تجاهلها بهدوء
        return
    user_id = update.effective_user.id
...
async def show_bot_stats_public(update: Update, context: ContextTypes.DEFAULT_TYPE):
    s = db.full_stats()
    settings = db.get_settings()
    boost_users = settings.get("boost_users", 0) or 0
    boost_tasks = settings.get("boost_tasks", 0) or 0
    boost_referrals = settings.get("boost_referrals", 0) or 0
    text = (
        f"🤖 إحصائيات عامة عن PR GRAM:\n"
        f"👥 المستخدمون: {fmt_lira(s['total_users'] + boost_users)}\n"
        f"📢 المهام النشطة: {fmt_lira(s['active_tasks'])}\n"
        f"✅ المهام المكتملة: {fmt_lira(s['completed_tasks'] + boost_tasks)}\n"
        f"🌟 إجمالي الإحالات: {fmt_lira(s['total_referrals'] + boost_referrals)}"
    )
    await update.message.reply_text(text)
...
async def show_competition(update: Update, context: ContextTypes.DEFAULT_TYPE):
    comp = db.get_active_competition()
    if not comp:
        await update.message.reply_text("🏆 لا توجد مسابقة حالياً، تابعنا لمعرفة موعد المسابقة القادمة!")
        return

    user_id = update.effective_user.id
    top3 = db.get_referral_leaderboard(3)
    prizes = {1: comp["prize_1"], 2: comp["prize_2"], 3: comp["prize_3"]}
    ...
    for rank in (1, 2, 3):
        ...
        prize = prizes[rank] or "—"
        lines.append(f"{medal} المركز {rank}: {prize}")
    lines.append("")
    lines.append("📊 المتصدرون حالياً:")
    if top3:
        for i, (uid, uname, rcount) in enumerate(top3, start=1):
            medal = RANK_MEDALS.get(i, f"{i}.")
            display_name = uname or str(uid)
            lines.append(f"{medal} {display_name} — {fmt_lira(rcount)} إحالة")
    else:
        lines.append("لا يوجد متصدرون بعد.")

    rank, my_count = db.get_user_referral_rank(user_id)
    lines.append("")
    if rank:
        lines.append(f"📍 ترتيبك أنت: #{fmt_lira(rank)} — لديك {fmt_lira(my_count)} إحالة")
    else:
        lines.append("📍 لسا ما عندك إحالات، شارك رابطك من قسم الترويج!")
...
async def handle_cheque_redeem_password(update: Update, context: ContextTypes.DEFAULT_TYPE, code):
    ...
    if fails >= CHEQUE_MAX_FAILS_BEFORE_CAPTCHA:
        ...
        context.user_data["awaiting"] = None
        await update.message.reply_text("⚠️ عدة محاولات خاطئة متتالية — تحقق أمني قبل ما تكمل.")
        await send_fruit_captcha(context, update.effective_user.id, "cheque")
        return
...
async def bottom_menu_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    routes = {
        ...
        "🏆 المسابقات": lambda: show_competition(update, context),
...
async def generic_text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.effective_user:
        return  # منشور قناة أو تحديث بدون مستخدم — تجاهله
...
telegram_app.add_handler(MessageHandler(filters.ALL, maintenance_gate), group=-2)
telegram_app.add_handler(CallbackQueryHandler(maintenance_gate), group=-2)
telegram_app.add_handler(MessageHandler(filters.ALL, security_flood_gate), group=-1)
telegram_app.add_handler(CallbackQueryHandler(security_flood_gate), group=-1)
...
telegram_app.add_handler(CallbackQueryHandler(fruit_captcha_click, pattern="^fruitcap:"))
...
async def startup_event():
    ...
    if manual_url:
        webhook_url = manual_url.rstrip("/") + "/webhook"
    elif render_url:
    ...
    if webhook_url:
        ...
        logger.warning("لم يتم العثور على رابط تلقائي (Render/Railway) — رجاءً اضبط WEBHOOK_URL يدوياً.")
