...
def init_db():
    ...
    c.execute("""
        CREATE TABLE IF NOT EXISTS settings (
            ...
            cheque_commission_percent INTEGER DEFAULT 4,
            cheque_min_amount INTEGER DEFAULT 10,
            cheque_password_max_length INTEGER DEFAULT 100,
            cheque_personal_max_amount INTEGER DEFAULT 3000,
            cheque_multi_min_amount INTEGER DEFAULT 100,
            cheque_multi_max_amount INTEGER DEFAULT 2500,
            sub_watch_days INTEGER DEFAULT 7,
            welcome_gift_points INTEGER DEFAULT 300
        )
    """)
    ...
    migrations = [
        ...
        ("settings", "cheque_commission_percent", "INTEGER DEFAULT 4"),
        ("settings", "cheque_min_amount", "INTEGER DEFAULT 10"),
        ("settings", "cheque_password_max_length", "INTEGER DEFAULT 100"),
        ("settings", "cheque_personal_max_amount", "INTEGER DEFAULT 3000"),
        ("settings", "cheque_multi_min_amount", "INTEGER DEFAULT 100"),
        ("settings", "cheque_multi_max_amount", "INTEGER DEFAULT 2500"),
        ("settings", "sub_watch_days", "INTEGER DEFAULT 7"),
        ("settings", "welcome_gift_points", "INTEGER DEFAULT 300"),
        ("tasks", "referral_link", "TEXT DEFAULT NULL"),
        ("settings", "boost_users", "INTEGER DEFAULT 0"),
        ("settings", "boost_tasks", "INTEGER DEFAULT 0"),
        ("settings", "boost_referrals", "INTEGER DEFAULT 0"),
        ("settings", "last_broadcast_total", "INTEGER DEFAULT 0"),
        ("settings", "last_broadcast_sent", "INTEGER DEFAULT 0"),
        ("settings", "last_broadcast_failed", "INTEGER DEFAULT 0"),
        ("settings", "last_broadcast_at", "TEXT DEFAULT NULL"),
        ("settings", "last_broadcast_preview", "TEXT DEFAULT NULL"),
    ]
...
_COMPETITION_EDITABLE_FIELDS = {"title", "description", "prize_1", "prize_2", "prize_3"}


def update_competition_field(competition_id, field, value):
    if field not in _COMPETITION_EDITABLE_FIELDS:
        raise ValueError(f"invalid competition field: {field}")
    conn = get_conn(); c = conn.cursor()
    c.execute(f"UPDATE competitions SET {field}=? WHERE competition_id=?", (value, competition_id))
    conn.commit(); conn.close()


def delete_competition(competition_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("DELETE FROM competitions WHERE competition_id=?", (competition_id,))
    conn.commit(); conn.close()


def record_broadcast_stats(total, sent, failed, preview):
    conn = get_conn(); c = conn.cursor()
    c.execute(
        "UPDATE settings SET last_broadcast_total=?, last_broadcast_sent=?, last_broadcast_failed=?, "
        "last_broadcast_at=?, last_broadcast_preview=? WHERE id=1",
        (total, sent, failed, now_str(), preview[:200] if preview else None)
    )
    conn.commit(); conn.close()
