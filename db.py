"""
db.py — طبقة قاعدة البيانات الكاملة للنظام الجديد (PR GRAM style)
PostgreSQL (عبر psycopg2) + دوال مساعدة لكل الجداول.

طبقة توافق: باقي هذا الملف مكتوب أصلاً بأسلوب sqlite3 (علامات '?' كـ placeholders،
و c.lastrowid لجلب آخر id متولد). للحفاظ على كل تلك الاستدعاءات كما هي بدون
إعادة كتابة كل استعلام يدوياً، الـ get_conn()/CompatCursor أدناه يترجمان تلقائياً:
  - '?' → '%s' (أسلوب psycopg2)
  - 'INSERT OR IGNORE INTO' → 'INSERT INTO ... ON CONFLICT DO NOTHING'
  - c.lastrowid: بعد أي INSERT بدون RETURNING، نضيف 'RETURNING *' تلقائياً
    ونخزن أول عمود من الصف الراجع (وهو دائماً عمود الـ PK لأنه أول عمود
    معرّف بكل جدول هنا) كقيمة lastrowid.
"""
import os
import re
import psycopg2
from datetime import datetime

DATABASE_URL = os.environ.get("DATABASE_URL")
if not DATABASE_URL:
    raise RuntimeError("لازم تضيف متغير البيئة DATABASE_URL (رابط قاعدة بيانات PostgreSQL) قبل تشغيل البوت")

_INSERT_IGNORE_RE = re.compile(r"(?i)insert\s+or\s+ignore\s+into")
_INSERT_RE = re.compile(r"(?i)^\s*insert\s+into")


class CompatCursor:
    """يلف مؤشر psycopg2 ليتصرف مثل مؤشر sqlite3 بالنسبة لبقية هذا الملف."""

    def __init__(self, cur):
        self._cur = cur
        self.lastrowid = None

    def execute(self, query, params=()):
        q = query
        is_insert_ignore = bool(_INSERT_IGNORE_RE.search(q))
        if is_insert_ignore:
            q = _INSERT_IGNORE_RE.sub("INSERT INTO", q)
        q = q.replace("?", "%s")
        is_insert = bool(_INSERT_RE.match(q))
        if is_insert_ignore:
            q = q.rstrip().rstrip(";") + " ON CONFLICT DO NOTHING"
        if is_insert and "returning" not in q.lower():
            q = q.rstrip().rstrip(";") + " RETURNING *"
        self._cur.execute(q, params)
        if is_insert:
            try:
                row = self._cur.fetchone()
                self.lastrowid = row[0] if row else None
            except psycopg2.ProgrammingError:
                self.lastrowid = None
        return self

    def fetchone(self):
        return self._cur.fetchone()

    def fetchall(self):
        return self._cur.fetchall()

    @property
    def description(self):
        return self._cur.description

    def close(self):
        self._cur.close()


class CompatConnection:
    def __init__(self, conn):
        self._conn = conn

    def cursor(self):
        return CompatCursor(self._conn.cursor())

    def commit(self):
        self._conn.commit()

    def close(self):
        self._conn.close()

CATEGORY_LABELS = {
    "group": "🔮 مجموعة",
    "channel": "📢 قناة",
    "bot": "🤖 بوت",
    "post": "👁️ منشور",
    "interaction": "🔥 تفاعلات",
    "premium": "⚡ شحن بريميوم",
}
CATEGORY_ORDER = ["group", "channel", "bot", "post", "interaction", "premium"]
AUTO_VERIFIABLE = {"group", "channel"}  # يتحقق منها تلقائياً عبر get_chat_member
MANUAL_VERIFIABLE = {"bot", "interaction"}  # تحتاج سكرين شوت + مراجعة صاحب المهمة
INSTANT_VERIFIABLE = {"post"}  # تُحتسب تلقائياً بعد مهلة عرض قصيرة

DEFAULT_CATEGORY_PRICES = {
    # (min_price, suggested_price, max_price)
    "group": (1000, 1343, 0),
    "channel": (750, 1810, 0),
    "bot": (900, 1200, 2500),
    "post": (100, 300, 0),
    "interaction": (500, 1000, 0),
}

# أنواع مهام "بوت" الفرعية
BOT_TYPES = {
    "normal": "▶️ بوت عادي",
    "conditions": "📝 بوت مع شروط إضافية",
    "webapp": "🌐 بوت Web App",
}
BOT_TYPE_ORDER = ["normal", "conditions", "webapp"]

# أسعار كل نوع بوت على حدة (أدنى، مقترح، أقصى) — يتحكم فيها الأدمن
DEFAULT_BOT_TYPE_PRICES = {
    "normal": (900, 1200, 2500),
    "conditions": (3000, 6899, 100000),
    "webapp": (1500, 3000, 50000),
}

# سجل كل رسائل/نصوص البوت القابلة للتعديل من لوحة الأدمن
# key: (label, description, default_text)
DEFAULT_MESSAGE_TEMPLATES = {
    "welcome_msg": (
        "👋 رسالة الترحيب",
        "هذه الرسالة تظهر للمستخدم بعد أن يضغط على /start لأول مرة أو في أي وقت.",
        "👋 أهلاً بك يا {name} في PR GRAM!\n"
        "📌 لدينا نظام إلزامي للتحقق من الاشتراك في محادثاتك.\n"
        "📈 يتيح لك نظام الإعلانات في PR GRAM جذب جمهور حقيقي والترويج لقنواتك ومجموعاتك وبوتاتك ومنشوراتك.\n"
        "💡 باستخدام البوت فإنك توافق تلقائياً على سياسة الخصوصية الخاصة بنا."
    ),
    "bot_task_normal_msg": (
        "🤖 رسالة فتح مهمة بوت عادي",
        "تظهر للمستخدم عندما يفتح مهمة من نوع «بوت عادي» من قسم الأرباح — بدون أي شروط إضافية.",
        "1️⃣ اذهب إلى البوت واضغط Start فقط.\nإذا ظهرت كابتشا أو اختيار لغة أكملها.\n📸 أرسل لقطة شاشة تثبت أنك فتحت البوت."
    ),
    "bot_task_conditions_msg": (
        "📝 رسالة فتح مهمة بوت بشروط",
        "تظهر للمستخدم عندما يفتح مهمة من نوع «بوت مع شروط إضافية» — تتضمن شروط صاحب المهمة.",
        "2️⃣ اذهب إلى البوت ونفّذ الشروط المطلوبة أدناه بالكامل.\n📸 بعد التنفيذ أرسل لقطة شاشة واضحة تثبت إنجازك."
    ),
    "bot_task_webapp_msg": (
        "🌐 رسالة فتح مهمة Web App",
        "تظهر للمستخدم عندما يفتح مهمة من نوع «بوت Web App» — يتطلب فتح التطبيق المصغّر داخل البوت.",
        "3️⃣ اذهب إلى البوت وافتح تطبيق الـ Web App من داخله وتفاعل معه.\n📸 أرسل لقطة شاشة تثبت فتحك للتطبيق."
    ),
    "verify_fail_msg": (
        "❌ رسالة فشل التحقق",
        "تظهر عندما يضغط المستخدم زر «التحقق من الاشتراك» قبل أن يكون مشتركاً فعلياً.",
        "❌ لم يتم التفعيل بعد.\nيرجى فتح الرابط أولاً ثم الضغط على زر «التحقق»."
    ),
    "after_photo_msg": (
        "📸 رسالة بعد استلام صورة عامة",
        "تظهر عندما يرسل المستخدم صورة والبوت لا ينتظر منه إثبات مهمة محددة في تلك اللحظة.",
        "📸 تم استلام الصورة، شكراً لمشاركتك!"
    ),
    "rules_text": (
        "👑 نص القواعد",
        "يظهر عند الضغط على زر «القواعد» بقسم الأرباح.",
        "يُمنع إلغاء الاشتراك من القنوات/المحادثات لمدة 7 أيام. مخالفة القاعدة تعني حظر المهام وإلغاء الأرباح."
    ),
    "instructions_text": (
        "📋 نص التعليمات",
        "يظهر عند الضغط على زر «📋 التعليمات» بالقائمة الرئيسية.",
        "📋 التعليمات: استخدم «📢 الترويج» لإنشاء مهام، و«🗂️ الأرباح» لكسب الرصيد."
    ),
    "useful_links": (
        "🔗 الروابط المفيدة",
        "يظهر عند الضغط على زر «🔗 روابط مفيدة» بالقائمة الرئيسية.",
        "🔗 لا يوجد روابط مضافة حالياً."
    ),
    "maintenance_msg": (
        "🛠️ رسالة إيقاف البوت",
        "تظهر لأي مستخدم عادي (غير أدمن) يحاول استخدام البوت وقت ما يكون موقوف مؤقتاً من الأدمن.",
        "عذراً، البوت قيد الصيانة حالياً. حاول لاحقاً 🙏"
    ),
    "mandatory_gate_msg": (
        "🚪 رسالة الاشتراك الإجباري عند /start",
        "تظهر للمستخدم أول ما يضغط /start إذا كان الاشتراك الإجباري بمجموعة/قناة مفعّلاً وما زال غير مشترك فيها.",
        "📌 عذراً، عليك الاشتراك بالقناة/المجموعة التالية أولاً لتتمكن من استخدام البوت."
    ),
    "main_group_promo_msg": (
        "📢 رسالة زر «انضم للمجموعة الرئيسية»",
        "تظهر عندما يضغط أي مستخدم على زر «📢 انضم للمجموعة الرئيسية» الظاهر بعد رسالة الترحيب.",
        "📢 انضم لمجموعتنا الرئيسية لمتابعة كل التحديثات والعروض!"
    ),
    "bot_referral_prompt_msg": (
        "🔗 رسالة طلب رابط إحالة البوت",
        "تظهر عند إنشاء مهمة بوت، بعد الضغط على «➕ إضافة رابط إحالة» — يطلب من صاحب المهمة يرسل رابط إحالة بوته.",
        "أرسل رابط الإحالة لبوتك:\nمثال:\nhttps://t.me/your_bot?start=123456789"
    ),
    "bot_referral_invalid_msg": (
        "❌ رسالة رابط إحالة خاطئ",
        "تظهر إذا أرسل صاحب مهمة البوت رابط إحالة لا يخص نفس البوت الذي اختاره لمهمته. المتغير {bot_username} يُستبدل تلقائياً باسم البوت الصحيح.",
        "❌ الرابط المرسل غير صحيح.\nتأكد من أنه يخص البوت الذي اخترته.\n@{bot_username}: يجب أن يحتوي الرابط على اسم المستخدم."
    ),
    "task_published_msg": (
        "✅ رسالة نجاح نشر المهمة",
        "تظهر بعد نشر أي مهمة ترويج بنجاح (قناة/مجموعة/بوت/منشور/تفاعل). المتغيرات: {task_id} رقم المهمة، {cost} قيمة الخصم.",
        "✅ تم نشر المهمة № {task_id}.\nتم خصم {cost} من رصيدك.\nتابع أخبارنا للمزيد."
    ),
}


def get_conn():
    return CompatConnection(psycopg2.connect(DATABASE_URL))


def init_db():
    conn = get_conn()
    c = conn.cursor()

    # ---------- المستخدمون ----------
    c.execute("""
        CREATE TABLE IF NOT EXISTS users (
            user_id BIGINT PRIMARY KEY,
            username TEXT,
            first_name TEXT,
            balance BIGINT DEFAULT 0,
            xp BIGINT DEFAULT 0,
            level TEXT DEFAULT 'Unknown',
            banned BIGINT DEFAULT 0,
            ban_reason TEXT DEFAULT NULL,
            ban_until TEXT DEFAULT NULL,
            strikes BIGINT DEFAULT 0,
            verified BIGINT DEFAULT 0,
            ref_by BIGINT DEFAULT NULL,
            referrals_count BIGINT DEFAULT 0,
            referral_earnings BIGINT DEFAULT 0,
            notifications_enabled BIGINT DEFAULT 1,
            language TEXT DEFAULT 'ar',
            joined_at TEXT,
            last_daily_gift_at TEXT DEFAULT NULL,
            no_commission_until TEXT DEFAULT NULL
        )
    """)

    # ---------- الإعدادات العامة (صف واحد) ----------
    c.execute("""
        CREATE TABLE IF NOT EXISTS settings (
            id BIGINT PRIMARY KEY,
            welcome_msg TEXT DEFAULT '👋 أهلاً بك في PR GRAM!',
            rules_text TEXT DEFAULT 'يُمنع إلغاء الاشتراك من القنوات/المحادثات لمدة 7 أيام.',
            commission_percent BIGINT DEFAULT 15,
            daily_gift_points BIGINT DEFAULT 50,
            ref_reward_premium BIGINT DEFAULT 10000,
            ref_reward_regular BIGINT DEFAULT 5000,
            ref_reward_mandatory BIGINT DEFAULT 3000,
            ref_income_percent_topup BIGINT DEFAULT 10,
            ref_income_percent_tasks BIGINT DEFAULT 3,
            stars_to_lira_rate BIGINT DEFAULT 100,
            stars_min_no_commission BIGINT DEFAULT 50,
            stars_no_commission_hours BIGINT DEFAULT 24,
            mandatory_sub_channel TEXT DEFAULT NULL,
            mandatory_gate_enabled BIGINT DEFAULT 0,
            useful_links TEXT DEFAULT NULL,
            instructions_text TEXT DEFAULT NULL,
            bot_enabled BIGINT DEFAULT 1,
            cheque_commission_percent BIGINT DEFAULT 4,
            cheque_min_amount BIGINT DEFAULT 10,
            cheque_password_max_length BIGINT DEFAULT 100,
            cheque_personal_max_amount BIGINT DEFAULT 3000,
            cheque_multi_min_amount BIGINT DEFAULT 100,
            cheque_multi_max_amount BIGINT DEFAULT 2500,
            sub_watch_days BIGINT DEFAULT 7,
            welcome_gift_points BIGINT DEFAULT 300
        )
    """)
    c.execute("INSERT OR IGNORE INTO settings (id) VALUES (1)")

    # ---------- أسعار الفئات ----------
    c.execute("""
        CREATE TABLE IF NOT EXISTS category_prices (
            category TEXT PRIMARY KEY,
            min_price BIGINT DEFAULT 100,
            suggested_price BIGINT DEFAULT 300,
            max_price BIGINT DEFAULT 0
        )
    """)
    for cat, (mn, sug, mx) in DEFAULT_CATEGORY_PRICES.items():
        c.execute(
            "INSERT OR IGNORE INTO category_prices (category, min_price, suggested_price, max_price) VALUES (?,?,?,?)",
            (cat, mn, sug, mx)
        )

    # ---------- أسعار أنواع البوت الفرعية (عادي / شروط / Web App) ----------
    c.execute("""
        CREATE TABLE IF NOT EXISTS bot_type_prices (
            bot_type TEXT PRIMARY KEY,
            min_price BIGINT DEFAULT 100,
            suggested_price BIGINT DEFAULT 300,
            max_price BIGINT DEFAULT 0
        )
    """)
    for bt, (mn, sug, mx) in DEFAULT_BOT_TYPE_PRICES.items():
        c.execute(
            "INSERT OR IGNORE INTO bot_type_prices (bot_type, min_price, suggested_price, max_price) VALUES (?,?,?,?)",
            (bt, mn, sug, mx)
        )

    # ---------- المهام / الحملات ----------
    c.execute("""
        CREATE TABLE IF NOT EXISTS tasks (
            task_id BIGSERIAL PRIMARY KEY,
            owner_id BIGINT,
            owner_username TEXT,
            category TEXT,
            unit_price BIGINT DEFAULT 0,
            target_count BIGINT DEFAULT 0,
            current_count BIGINT DEFAULT 0,
            total_cost BIGINT DEFAULT 0,
            payment_method TEXT DEFAULT 'lira',
            commission_percent BIGINT DEFAULT 15,
            target_chat_id TEXT DEFAULT NULL,
            target_chat_title TEXT DEFAULT NULL,
            link TEXT DEFAULT NULL,
            bot_username TEXT DEFAULT NULL,
            extra_conditions TEXT DEFAULT NULL,
            requires_review BIGINT DEFAULT 0,
            join_request_mode BIGINT DEFAULT 0,
            interaction_mode TEXT DEFAULT NULL,
            fixed_emoji TEXT DEFAULT NULL,
            bot_type TEXT DEFAULT NULL,
            filter_account_type TEXT DEFAULT 'all',
            filter_audience TEXT DEFAULT 'all',
            notify_owner BIGINT DEFAULT 1,
            status TEXT DEFAULT 'active',
            source_message_id BIGINT DEFAULT NULL,
            created_at TEXT
        )
    """)

    # ---------- تنفيذ المهام من طرف المستخدمين ----------
    c.execute("""
        CREATE TABLE IF NOT EXISTS task_completions (
            task_id BIGINT,
            user_id BIGINT,
            status TEXT DEFAULT 'in_progress',
            proof_file_id TEXT DEFAULT NULL,
            reward_amount BIGINT DEFAULT 0,
            started_at TEXT,
            submitted_at TEXT DEFAULT NULL,
            resolved_at TEXT DEFAULT NULL,
            review_note TEXT DEFAULT NULL,
            PRIMARY KEY (task_id, user_id)
        )
    """)

    # ---------- روابط إحالة خاصة يُنشئها الأدمن بقيمة حرة ----------
    c.execute("""
        CREATE TABLE IF NOT EXISTS admin_ref_links (
            link_id BIGSERIAL PRIMARY KEY,
            code TEXT UNIQUE,
            title TEXT,
            reward_amount BIGINT DEFAULT 0,
            max_uses BIGINT DEFAULT 0,
            uses_count BIGINT DEFAULT 0,
            active BIGINT DEFAULT 1,
            created_by BIGINT,
            created_at TEXT
        )
    """)
    c.execute("""
        CREATE TABLE IF NOT EXISTS admin_ref_link_uses (
            link_id BIGINT,
            user_id BIGINT,
            used_at TEXT,
            PRIMARY KEY (link_id, user_id)
        )
    """)

    # ---------- المسابقات ----------
    c.execute("""
        CREATE TABLE IF NOT EXISTS competitions (
            competition_id BIGSERIAL PRIMARY KEY,
            title TEXT,
            description TEXT,
            prize_1 TEXT,
            prize_2 TEXT,
            prize_3 TEXT,
            created_by BIGINT,
            created_at TEXT,
            active BIGINT DEFAULT 1
        )
    """)

    # ---------- سجل مدفوعات النجوم ----------
    c.execute("""
        CREATE TABLE IF NOT EXISTS stars_payments (
            payment_id BIGSERIAL PRIMARY KEY,
            user_id BIGINT,
            purpose TEXT,
            payload TEXT,
            stars_amount BIGINT,
            related_task_id BIGINT DEFAULT NULL,
            status TEXT DEFAULT 'pending',
            created_at TEXT,
            paid_at TEXT DEFAULT NULL
        )
    """)

    # ---------- سجل المعاملات (لِلَّإحصائيات) ----------
    c.execute("""
        CREATE TABLE IF NOT EXISTS transactions (
            tx_id BIGSERIAL PRIMARY KEY,
            user_id BIGINT,
            amount BIGINT,
            kind TEXT,
            note TEXT,
            created_at TEXT
        )
    """)

    # ---------- الشيكات (تحويل نقاط عبر رابط، شخصي أو متعدد) ----------
    c.execute("""
        CREATE TABLE IF NOT EXISTS cheques (
            cheque_id BIGSERIAL PRIMARY KEY,
            code TEXT UNIQUE,
            creator_id BIGINT,
            kind TEXT DEFAULT 'personal',
            amount BIGINT DEFAULT 0,
            commission_percent BIGINT DEFAULT 0,
            total_cost BIGINT DEFAULT 0,
            max_uses BIGINT DEFAULT 1,
            uses_count BIGINT DEFAULT 0,
            password TEXT DEFAULT NULL,
            comment TEXT DEFAULT NULL,
            image_file_id TEXT DEFAULT NULL,
            require_channel TEXT DEFAULT NULL,
            notifications_enabled BIGINT DEFAULT 1,
            status TEXT DEFAULT 'active',
            created_at TEXT
        )
    """)
    c.execute("""
        CREATE TABLE IF NOT EXISTS cheque_redemptions (
            cheque_id BIGINT,
            user_id BIGINT,
            redeemed_at TEXT,
            PRIMARY KEY (cheque_id, user_id)
        )
    """)

    # ---------- مراقبة الاشتراك الإجباري لمدة 7 أيام لمهام القنوات/المجموعات ----------
    c.execute("""
        CREATE TABLE IF NOT EXISTS subscription_watches (
            user_id BIGINT,
            chat_id TEXT,
            task_id BIGINT,
            joined_at TEXT,
            expires_at TEXT,
            PRIMARY KEY (user_id, chat_id)
        )
    """)
    c.execute("""
        CREATE TABLE IF NOT EXISTS subscription_violations (
            violation_id BIGSERIAL PRIMARY KEY,
            user_id BIGINT,
            chat_id TEXT,
            chat_title TEXT,
            task_id BIGINT,
            fine_amount BIGINT,
            status TEXT DEFAULT 'pending',
            created_at TEXT,
            resolved_at TEXT DEFAULT NULL
        )
    """)

    # ---------- نصوص/رسائل البوت القابلة للتعديل من الأدمن ----------
    c.execute("""
        CREATE TABLE IF NOT EXISTS message_templates (
            key TEXT PRIMARY KEY,
            label TEXT,
            description TEXT,
            content TEXT
        )
    """)
    for key, (label, description, default) in DEFAULT_MESSAGE_TEMPLATES.items():
        c.execute(
            "INSERT OR IGNORE INTO message_templates (key, label, description, content) VALUES (?,?,?,?)",
            (key, label, description, default)
        )

    # ---------- أكواد الهدايا ----------
    c.execute("""
        CREATE TABLE IF NOT EXISTS gift_codes (
            code_id BIGSERIAL PRIMARY KEY,
            code TEXT UNIQUE,
            points BIGINT DEFAULT 0,
            max_uses BIGINT DEFAULT 1,
            uses_count BIGINT DEFAULT 0,
            active BIGINT DEFAULT 1,
            created_by BIGINT,
            created_at TEXT
        )
    """)
    c.execute("""
        CREATE TABLE IF NOT EXISTS gift_code_uses (
            code_id BIGINT,
            user_id BIGINT,
            used_at TEXT,
            PRIMARY KEY (code_id, user_id)
        )
    """)

    # ---------- ترحيل الأعمدة الجديدة على جداول قديمة ----------
    migrations = [
        ("tasks", "bot_type", "TEXT DEFAULT NULL"),
        ("tasks", "source_message_id", "BIGINT DEFAULT NULL"),
        ("settings", "mandatory_gate_enabled", "BIGINT DEFAULT 0"),
        ("settings", "bot_enabled", "BIGINT DEFAULT 1"),
        ("settings", "cheque_commission_percent", "BIGINT DEFAULT 4"),
        ("settings", "cheque_min_amount", "BIGINT DEFAULT 10"),
        ("settings", "cheque_password_max_length", "BIGINT DEFAULT 100"),
        ("settings", "cheque_personal_max_amount", "BIGINT DEFAULT 3000"),
        ("settings", "cheque_multi_min_amount", "BIGINT DEFAULT 100"),
        ("settings", "cheque_multi_max_amount", "BIGINT DEFAULT 2500"),
        ("settings", "sub_watch_days", "BIGINT DEFAULT 7"),
        ("settings", "welcome_gift_points", "BIGINT DEFAULT 300"),
        ("tasks", "referral_link", "TEXT DEFAULT NULL"),
        ("settings", "boost_users", "BIGINT DEFAULT 0"),
        ("settings", "boost_tasks", "BIGINT DEFAULT 0"),
        ("settings", "boost_referrals", "BIGINT DEFAULT 0"),
    ]
    for table, column, col_def in migrations:
        c.execute(f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS {column} {col_def}")

    conn.commit()
    conn.close()


def now_str():
    return datetime.now().strftime("%Y-%m-%d %H:%M:%S")


# ==================== المستخدمون ====================

def ensure_user(user_id, username, first_name, ref_by=None):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT user_id FROM users WHERE user_id=?", (user_id,))
    if c.fetchone():
        c.execute("UPDATE users SET username=?, first_name=? WHERE user_id=?", (username, first_name, user_id))
        conn.commit(); conn.close()
        return False
    c.execute(
        "INSERT INTO users (user_id, username, first_name, joined_at, ref_by) VALUES (?,?,?,?,?)",
        (user_id, username, first_name, now_str(), ref_by)
    )
    conn.commit(); conn.close()
    return True


def get_user(user_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM users WHERE user_id=?", (user_id,))
    row = c.fetchone()
    if not row:
        conn.close(); return None
    cols = [d[0] for d in c.description]
    conn.close()
    return dict(zip(cols, row))


def log_tx(user_id, amount, kind, note=""):
    conn = get_conn(); c = conn.cursor()
    c.execute("INSERT INTO transactions (user_id, amount, kind, note, created_at) VALUES (?,?,?,?,?)",
              (user_id, amount, kind, note, now_str()))
    conn.commit(); conn.close()


def add_balance(user_id, amount, kind="adjust", note=""):
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE users SET balance = balance + ? WHERE user_id=?", (amount, user_id))
    conn.commit(); conn.close()
    log_tx(user_id, amount, kind, note)


def set_field(table, user_id_col, user_id, field, value):
    conn = get_conn(); c = conn.cursor()
    c.execute(f"UPDATE {table} SET {field}=? WHERE {user_id_col}=?", (value, user_id))
    conn.commit(); conn.close()


def toggle_ban(user_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT banned FROM users WHERE user_id=?", (user_id,))
    row = c.fetchone()
    if not row:
        conn.close(); return None
    new_val = 0 if row[0] else 1
    c.execute("UPDATE users SET banned=? WHERE user_id=?", (new_val, user_id))
    conn.commit(); conn.close()
    return new_val


def get_all_active_user_ids():
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT user_id FROM users WHERE banned=0")
    rows = [r[0] for r in c.fetchall()]
    conn.close()
    return rows


def count_users():
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT COUNT(*) FROM users")
    n = c.fetchone()[0]
    conn.close()
    return n


def get_users_page(offset, limit):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT user_id, username, balance, banned FROM users ORDER BY joined_at DESC LIMIT ? OFFSET ?",
              (limit, offset))
    rows = c.fetchall()
    conn.close()
    return rows


# ==================== الإعدادات ====================

def get_settings():
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM settings WHERE id=1")
    row = c.fetchone()
    cols = [d[0] for d in c.description]
    conn.close()
    return dict(zip(cols, row))


def update_setting(field, value):
    conn = get_conn(); c = conn.cursor()
    c.execute(f"UPDATE settings SET {field}=? WHERE id=1", (value,))
    conn.commit(); conn.close()


def add_boost(field, amount):
    """field يجب أن يكون أحد: boost_users, boost_tasks, boost_referrals (قيمة ثابتة من كودنا وليست من مدخلات المستخدم)."""
    conn = get_conn(); c = conn.cursor()
    c.execute(f"UPDATE settings SET {field} = {field} + ? WHERE id=1", (amount,))
    conn.commit(); conn.close()


def reset_boost(field):
    conn = get_conn(); c = conn.cursor()
    c.execute(f"UPDATE settings SET {field} = 0 WHERE id=1")
    conn.commit(); conn.close()


# ==================== أسعار الفئات ====================

def get_category_price(category):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT min_price, suggested_price, max_price FROM category_prices WHERE category=?", (category,))
    row = c.fetchone()
    conn.close()
    if not row:
        return DEFAULT_CATEGORY_PRICES.get(category, (100, 300, 0))
    return row


def set_category_price(category, mn, sug, mx):
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE category_prices SET min_price=?, suggested_price=?, max_price=? WHERE category=?",
              (mn, sug, mx, category))
    conn.commit(); conn.close()


def get_all_category_prices():
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT category, min_price, suggested_price, max_price FROM category_prices")
    rows = c.fetchall()
    conn.close()
    return rows


# ==================== أسعار أنواع البوت الفرعية ====================

def get_bot_type_price(bot_type):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT min_price, suggested_price, max_price FROM bot_type_prices WHERE bot_type=?", (bot_type,))
    row = c.fetchone()
    conn.close()
    if not row:
        return DEFAULT_BOT_TYPE_PRICES.get(bot_type, (100, 300, 0))
    return row


def set_bot_type_price(bot_type, mn, sug, mx):
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE bot_type_prices SET min_price=?, suggested_price=?, max_price=? WHERE bot_type=?",
              (mn, sug, mx, bot_type))
    conn.commit(); conn.close()


# ==================== نصوص/رسائل البوت (Message Templates) ====================

def get_all_templates():
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT key, label, description, content FROM message_templates")
    rows = c.fetchall()
    conn.close()
    return rows


def get_template(key):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT label, description, content FROM message_templates WHERE key=?", (key,))
    row = c.fetchone()
    conn.close()
    if not row:
        return None
    return {"label": row[0], "description": row[1], "content": row[2]}


def set_template_content(key, content):
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE message_templates SET content=? WHERE key=?", (content, key))
    conn.commit(); conn.close()


# ==================== أكواد الهدايا ====================

def create_gift_code(code, points, max_uses, created_by):
    conn = get_conn(); c = conn.cursor()
    c.execute(
        "INSERT INTO gift_codes (code, points, max_uses, created_by, created_at) VALUES (?,?,?,?,?)",
        (code, points, max_uses, created_by, now_str())
    )
    conn.commit()
    code_id = c.lastrowid
    conn.close()
    return code_id


def get_gift_code(code):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM gift_codes WHERE code=?", (code,))
    row = c.fetchone()
    if not row:
        conn.close(); return None
    cols = [d[0] for d in c.description]
    conn.close()
    return dict(zip(cols, row))


def list_gift_codes():
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT code_id, code, points, max_uses, uses_count, active FROM gift_codes ORDER BY created_at DESC")
    rows = c.fetchall()
    conn.close()
    return rows


def toggle_gift_code(code_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT active FROM gift_codes WHERE code_id=?", (code_id,))
    row = c.fetchone()
    if not row:
        conn.close(); return None
    new_val = 0 if row[0] else 1
    c.execute("UPDATE gift_codes SET active=? WHERE code_id=?", (new_val, code_id))
    conn.commit(); conn.close()
    return new_val


def redeem_gift_code(code, user_id):
    """يرجع (نجاح, نقاط أو سبب الفشل: invalid/exhausted/already_used)"""
    gc = get_gift_code(code)
    if not gc or not gc["active"]:
        return False, "invalid"
    if gc["max_uses"] and gc["uses_count"] >= gc["max_uses"]:
        return False, "exhausted"
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT 1 FROM gift_code_uses WHERE code_id=? AND user_id=?", (gc["code_id"], user_id))
    if c.fetchone():
        conn.close(); return False, "already_used"
    c.execute("INSERT INTO gift_code_uses (code_id, user_id, used_at) VALUES (?,?,?)",
              (gc["code_id"], user_id, now_str()))
    c.execute("UPDATE gift_codes SET uses_count = uses_count + 1 WHERE code_id=?", (gc["code_id"],))
    conn.commit(); conn.close()
    return True, gc["points"]


# ==================== الشيكات (تحويل نقاط عبر رابط) ====================

def create_cheque(code, kind, creator_id, amount, commission_percent, total_cost, max_uses,
                   password=None, comment=None, require_channel=None):
    conn = get_conn(); c = conn.cursor()
    c.execute(
        "INSERT INTO cheques (code, kind, creator_id, amount, commission_percent, total_cost, max_uses, "
        "password, comment, require_channel, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
        (code, kind, creator_id, amount, commission_percent, total_cost, max_uses,
         password, comment, require_channel, now_str())
    )
    conn.commit()
    cheque_id = c.lastrowid
    conn.close()
    return cheque_id


def get_cheque(cheque_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM cheques WHERE cheque_id=?", (cheque_id,))
    row = c.fetchone()
    if not row:
        conn.close(); return None
    cols = [d[0] for d in c.description]
    conn.close()
    return dict(zip(cols, row))


def get_cheque_by_code(code):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM cheques WHERE code=?", (code,))
    row = c.fetchone()
    if not row:
        conn.close(); return None
    cols = [d[0] for d in c.description]
    conn.close()
    return dict(zip(cols, row))


def list_cheques_by_owner(owner_id, kind=None):
    conn = get_conn(); c = conn.cursor()
    if kind:
        c.execute("SELECT * FROM cheques WHERE creator_id=? AND kind=? AND status!='deleted' ORDER BY created_at DESC",
                  (owner_id, kind))
    else:
        c.execute("SELECT * FROM cheques WHERE creator_id=? AND status!='deleted' ORDER BY created_at DESC", (owner_id,))
    rows = c.fetchall()
    cols = [d[0] for d in c.description]
    conn.close()
    return [dict(zip(cols, r)) for r in rows]


def update_cheque_field(cheque_id, field, value):
    conn = get_conn(); c = conn.cursor()
    c.execute(f"UPDATE cheques SET {field}=? WHERE cheque_id=?", (value, cheque_id))
    conn.commit(); conn.close()


def cheque_already_redeemed(cheque_id, user_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT 1 FROM cheque_redemptions WHERE cheque_id=? AND user_id=?", (cheque_id, user_id))
    row = c.fetchone()
    conn.close()
    return row is not None


def redeem_cheque(code, user_id):
    """يرجع (نجاح, المبلغ أو سبب الفشل: invalid/exhausted/already_used/needs_password/wrong_password)"""
    ch = get_cheque_by_code(code)
    if not ch or ch["status"] != "active":
        return False, "invalid"
    if ch["max_uses"] and ch["uses_count"] >= ch["max_uses"]:
        return False, "exhausted"
    if cheque_already_redeemed(ch["cheque_id"], user_id):
        return False, "already_used"
    conn = get_conn(); c = conn.cursor()
    c.execute("INSERT INTO cheque_redemptions (cheque_id, user_id, redeemed_at) VALUES (?,?,?)",
              (ch["cheque_id"], user_id, now_str()))
    c.execute("UPDATE cheques SET uses_count = uses_count + 1 WHERE cheque_id=?", (ch["cheque_id"],))
    new_uses = ch["uses_count"] + 1
    if ch["max_uses"] and new_uses >= ch["max_uses"]:
        c.execute("UPDATE cheques SET status='used' WHERE cheque_id=?", (ch["cheque_id"],))
    conn.commit(); conn.close()
    return True, ch["amount"]


# ==================== روابط الإحالة الخاصة (أدمن) ====================

def create_admin_ref_link(code, title, reward_amount, max_uses, created_by):
    conn = get_conn(); c = conn.cursor()
    c.execute(
        "INSERT INTO admin_ref_links (code, title, reward_amount, max_uses, created_by, created_at) VALUES (?,?,?,?,?,?)",
        (code, title, reward_amount, max_uses, created_by, now_str())
    )
    conn.commit()
    link_id = c.lastrowid
    conn.close()
    return link_id


def get_admin_ref_link_by_code(code):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM admin_ref_links WHERE code=?", (code,))
    row = c.fetchone()
    if not row:
        conn.close(); return None
    cols = [d[0] for d in c.description]
    conn.close()
    return dict(zip(cols, row))


def list_admin_ref_links():
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT link_id, code, title, reward_amount, max_uses, uses_count, active FROM admin_ref_links ORDER BY created_at DESC")
    rows = c.fetchall()
    conn.close()
    return rows


def toggle_admin_ref_link(link_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT active FROM admin_ref_links WHERE link_id=?", (link_id,))
    row = c.fetchone()
    if not row:
        conn.close(); return None
    new_val = 0 if row[0] else 1
    c.execute("UPDATE admin_ref_links SET active=? WHERE link_id=?", (new_val, link_id))
    conn.commit(); conn.close()
    return new_val


def use_admin_ref_link(code, user_id):
    """يرجع (نجاح, المكافأة أو سبب الفشل)"""
    link = get_admin_ref_link_by_code(code)
    if not link or not link["active"]:
        return False, "invalid"
    if link["max_uses"] and link["uses_count"] >= link["max_uses"]:
        return False, "exhausted"
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT 1 FROM admin_ref_link_uses WHERE link_id=? AND user_id=?", (link["link_id"], user_id))
    if c.fetchone():
        conn.close(); return False, "already_used"
    c.execute("INSERT INTO admin_ref_link_uses (link_id, user_id, used_at) VALUES (?,?,?)",
              (link["link_id"], user_id, now_str()))
    c.execute("UPDATE admin_ref_links SET uses_count = uses_count + 1 WHERE link_id=?", (link["link_id"],))
    conn.commit(); conn.close()
    return True, link["reward_amount"]


# ==================== المسابقات ====================

def create_competition(title, description, prize_1, prize_2, prize_3, created_by):
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE competitions SET active=0")
    c.execute(
        "INSERT INTO competitions (title, description, prize_1, prize_2, prize_3, created_by, created_at, active) "
        "VALUES (?,?,?,?,?,?,?,1)",
        (title, description, prize_1, prize_2, prize_3, created_by, now_str())
    )
    conn.commit()
    cid = c.lastrowid
    conn.close()
    return cid


def get_active_competition():
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM competitions WHERE active=1 ORDER BY created_at DESC LIMIT 1")
    row = c.fetchone()
    if not row:
        conn.close(); return None
    cols = [d[0] for d in c.description]
    conn.close()
    return dict(zip(cols, row))


def get_referral_leaderboard(limit=3):
    """يرجع أعلى المستخدمين عدد إحالات (غير المحظورين)، كل عنصر (user_id, username, referrals_count)."""
    conn = get_conn(); c = conn.cursor()
    c.execute(
        "SELECT user_id, username, referrals_count FROM users WHERE banned=0 "
        "ORDER BY referrals_count DESC, user_id ASC LIMIT ?",
        (limit,)
    )
    rows = c.fetchall()
    conn.close()
    return rows


def get_user_referral_rank(user_id):
    """يرجع (الترتيب، عدد الإحالات) للمستخدم ضمن كل المستخدمين غير المحظورين."""
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT referrals_count FROM users WHERE user_id=?", (user_id,))
    row = c.fetchone()
    if not row:
        conn.close(); return None, 0
    my_count = row[0]
    c.execute("SELECT COUNT(*) FROM users WHERE banned=0 AND referrals_count > ?", (my_count,))
    higher = c.fetchone()[0]
    conn.close()
    return higher + 1, my_count


def set_referrals_count(user_id, count):
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE users SET referrals_count=? WHERE user_id=?", (count, user_id))
    conn.commit(); conn.close()


# ==================== المهام ====================

def create_task(**kw):
    conn = get_conn(); c = conn.cursor()
    fields = ["owner_id", "owner_username", "category", "unit_price", "target_count", "total_cost",
              "payment_method", "commission_percent", "target_chat_id", "target_chat_title", "link",
              "bot_username", "extra_conditions", "requires_review", "join_request_mode",
              "interaction_mode", "fixed_emoji", "bot_type", "source_message_id", "referral_link",
              "status", "created_at"]
    kw.setdefault("status", "active")
    kw["created_at"] = now_str()
    cols = ", ".join(fields)
    placeholders = ", ".join(["?"] * len(fields))
    values = [kw.get(f) for f in fields]
    c.execute(f"INSERT INTO tasks ({cols}) VALUES ({placeholders})", values)
    task_id = c.lastrowid
    conn.commit(); conn.close()
    return task_id


def get_task_by_owner_and_bot(owner_id, bot_username):
    conn = get_conn(); c = conn.cursor()
    c.execute(
        "SELECT * FROM tasks WHERE owner_id=? AND bot_username=? AND category='bot' AND status IN ('active','paused') "
        "ORDER BY created_at DESC LIMIT 1",
        (owner_id, bot_username)
    )
    row = c.fetchone()
    if not row:
        conn.close(); return None
    cols = [d[0] for d in c.description]
    conn.close()
    return dict(zip(cols, row))


def get_task(task_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM tasks WHERE task_id=?", (task_id,))
    row = c.fetchone()
    if not row:
        conn.close(); return None
    cols = [d[0] for d in c.description]
    conn.close()
    return dict(zip(cols, row))


def update_task_field(task_id, field, value):
    conn = get_conn(); c = conn.cursor()
    c.execute(f"UPDATE tasks SET {field}=? WHERE task_id=?", (value, task_id))
    conn.commit(); conn.close()


def increment_task_progress(task_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE tasks SET current_count = current_count + 1 WHERE task_id=?", (task_id,))
    c.execute("SELECT target_count, current_count FROM tasks WHERE task_id=?", (task_id,))
    tgt, cur = c.fetchone()
    if cur >= tgt:
        c.execute("UPDATE tasks SET status='completed' WHERE task_id=?", (task_id,))
    conn.commit(); conn.close()
    return cur, tgt


def get_active_tasks(category, exclude_owner=None):
    conn = get_conn(); c = conn.cursor()
    c.execute(
        "SELECT * FROM tasks WHERE status='active' AND category=? AND current_count < target_count ORDER BY unit_price DESC",
        (category,)
    )
    rows = c.fetchall()
    cols = [d[0] for d in c.description]
    conn.close()
    result = [dict(zip(cols, r)) for r in rows]
    if exclude_owner:
        result = [t for t in result if t["owner_id"] != exclude_owner]
    return result


def count_tasks_by_category():
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT category, COUNT(*) FROM tasks WHERE status='active' AND current_count < target_count GROUP BY category")
    rows = dict(c.fetchall())
    conn.close()
    return rows


def get_tasks_by_owner(owner_id, status_filter=None):
    conn = get_conn(); c = conn.cursor()
    if status_filter:
        c.execute("SELECT * FROM tasks WHERE owner_id=? AND status=? ORDER BY created_at DESC", (owner_id, status_filter))
    else:
        c.execute("SELECT * FROM tasks WHERE owner_id=? ORDER BY created_at DESC", (owner_id,))
    rows = c.fetchall()
    cols = [d[0] for d in c.description]
    conn.close()
    return [dict(zip(cols, r)) for r in rows]


def delete_task(task_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE tasks SET status='deleted' WHERE task_id=?", (task_id,))
    conn.commit(); conn.close()


def set_task_status(task_id, status):
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE tasks SET status=? WHERE task_id=?", (status, task_id))
    conn.commit(); conn.close()


# ==================== تنفيذ المهام ====================

def get_completion(task_id, user_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM task_completions WHERE task_id=? AND user_id=?", (task_id, user_id))
    row = c.fetchone()
    if not row:
        conn.close(); return None
    cols = [d[0] for d in c.description]
    conn.close()
    return dict(zip(cols, row))


def start_completion(task_id, user_id):
    row = get_completion(task_id, user_id)
    conn = get_conn(); c = conn.cursor()
    if row is None or row["status"] == "rejected":
        c.execute(
            "INSERT INTO task_completions (task_id, user_id, status, started_at) VALUES (?,?, 'in_progress', ?) "
            "ON CONFLICT(task_id, user_id) DO UPDATE SET status='in_progress', started_at=excluded.started_at, "
            "proof_file_id=NULL, submitted_at=NULL, resolved_at=NULL, review_note=NULL",
            (task_id, user_id, now_str())
        )
        conn.commit(); conn.close()
        return "in_progress"
    conn.close()
    return row["status"]


def submit_proof(task_id, user_id, file_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE task_completions SET status='submitted', proof_file_id=?, submitted_at=? WHERE task_id=? AND user_id=?",
              (file_id, now_str(), task_id, user_id))
    conn.commit(); conn.close()


def resolve_completion(task_id, user_id, status, note=None):
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE task_completions SET status=?, resolved_at=?, review_note=? WHERE task_id=? AND user_id=?",
              (status, now_str(), note, task_id, user_id))
    conn.commit(); conn.close()


def get_latest_in_progress(user_id):
    """يرجع آخر مهمة بدأ المستخدم تنفيذها (in_progress) أو طُلب منه إعادة تنفيذها (review_requested) وبانتظار إثبات صورة."""
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT task_id FROM task_completions WHERE user_id=? AND status IN ('in_progress','review_requested') "
              "ORDER BY started_at DESC LIMIT 1", (user_id,))
    row = c.fetchone()
    conn.close()
    return row[0] if row else None


def user_task_status(task_id, user_id):
    row = get_completion(task_id, user_id)
    return row["status"] if row else None


def count_completions_by_status():
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT status, COUNT(*) FROM task_completions GROUP BY status")
    rows = dict(c.fetchall())
    conn.close()
    return rows


# ==================== إحصائيات شاملة ====================

# ==================== مراقبة الاشتراك الإجباري (منع إلغاء الاشتراك المبكر) ====================

def upsert_subscription_watch(user_id, chat_id, task_id, watch_days):
    joined = now_str()
    expires = (datetime.strptime(joined, "%Y-%m-%d %H:%M:%S") + timedelta(days=watch_days)).strftime("%Y-%m-%d %H:%M:%S")
    conn = get_conn(); c = conn.cursor()
    c.execute(
        "INSERT INTO subscription_watches (user_id, chat_id, task_id, joined_at, expires_at) VALUES (?,?,?,?,?) "
        "ON CONFLICT(user_id, chat_id) DO UPDATE SET task_id=excluded.task_id, joined_at=excluded.joined_at, expires_at=excluded.expires_at",
        (user_id, str(chat_id), task_id, joined, expires)
    )
    conn.commit(); conn.close()


def get_subscription_watch(user_id, chat_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM subscription_watches WHERE user_id=? AND chat_id=?", (user_id, str(chat_id)))
    row = c.fetchone()
    if not row:
        conn.close(); return None
    cols = [d[0] for d in c.description]
    conn.close()
    return dict(zip(cols, row))


def remove_subscription_watch(user_id, chat_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("DELETE FROM subscription_watches WHERE user_id=? AND chat_id=?", (user_id, str(chat_id)))
    conn.commit(); conn.close()


def create_violation(user_id, chat_id, chat_title, task_id, fine_amount):
    conn = get_conn(); c = conn.cursor()
    c.execute(
        "INSERT INTO subscription_violations (user_id, chat_id, chat_title, task_id, fine_amount, status, created_at) "
        "VALUES (?,?,?,?,?, 'pending', ?)",
        (user_id, str(chat_id), chat_title, task_id, fine_amount, now_str())
    )
    conn.commit()
    vid = c.lastrowid
    conn.close()
    return vid


def get_pending_violations(user_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM subscription_violations WHERE user_id=? AND status='pending' ORDER BY created_at", (user_id,))
    rows = c.fetchall()
    cols = [d[0] for d in c.description]
    conn.close()
    return [dict(zip(cols, r)) for r in rows]


def get_violation(violation_id):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM subscription_violations WHERE violation_id=?", (violation_id,))
    row = c.fetchone()
    if not row:
        conn.close(); return None
    cols = [d[0] for d in c.description]
    conn.close()
    return dict(zip(cols, row))


def resolve_violation(violation_id, status):
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE subscription_violations SET status=?, resolved_at=? WHERE violation_id=?",
              (status, now_str(), violation_id))
    conn.commit(); conn.close()


def full_stats():
    conn = get_conn(); c = conn.cursor()
    stats = {}
    c.execute("SELECT COUNT(*) FROM users"); stats["total_users"] = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM users WHERE banned=0"); stats["active_users"] = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM users WHERE banned=1"); stats["banned_users"] = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM users WHERE verified=1"); stats["verified_users"] = c.fetchone()[0]
    c.execute("SELECT SUM(balance) FROM users"); stats["total_balance"] = c.fetchone()[0] or 0
    c.execute("SELECT COUNT(*) FROM users WHERE joined_at::date = CURRENT_DATE"); stats["new_today"] = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM tasks WHERE status='active'"); stats["active_tasks"] = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM tasks WHERE status='completed'"); stats["completed_tasks"] = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM tasks"); stats["total_tasks"] = c.fetchone()[0]
    c.execute("SELECT SUM(total_cost) FROM tasks"); stats["total_spent_on_tasks"] = c.fetchone()[0] or 0
    c.execute("SELECT COUNT(*) FROM task_completions WHERE status='approved'"); stats["approved_completions"] = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM task_completions WHERE status IN ('submitted','review_requested')")
    stats["pending_reviews"] = c.fetchone()[0]
    c.execute("SELECT SUM(stars_amount) FROM stars_payments WHERE status='paid'")
    stats["total_stars_revenue"] = c.fetchone()[0] or 0
    c.execute("SELECT COUNT(*) FROM stars_payments WHERE status='paid'"); stats["stars_payments_count"] = c.fetchone()[0]
    c.execute("SELECT SUM(referrals_count) FROM users"); stats["total_referrals"] = c.fetchone()[0] or 0
    c.execute("SELECT category, COUNT(*) FROM tasks GROUP BY category")
    stats["tasks_by_category"] = dict(c.fetchall())
    c.execute("SELECT username, balance FROM users ORDER BY balance DESC LIMIT 10")
    stats["top_balances"] = c.fetchall()
    c.execute("SELECT username, referrals_count FROM users ORDER BY referrals_count DESC LIMIT 10")
    stats["top_referrers"] = c.fetchall()
    conn.close()
    return stats
