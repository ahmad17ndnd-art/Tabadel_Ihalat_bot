...
async def security_flood_gate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    يطلب كابتشا رياضية بسيطة قبل ما يسمحله يكمل. غير مطبّق على الأدمن.
    """
    user = update.effective_user
    if not user or config.is_admin(user.id):
        return
    if context.user_data.get("awaiting") == "security_captcha":
        return  # هاي الرسالة هي جواب الكابتشا نفسها — خليها توصل لمعالجها
    ...
    context.user_data["security_captcha_answer"] = a + b
    context.user_data["awaiting"] = "security_captcha"
    text = f"⚠️ لاحظنا نشاط سريع غير معتاد من حسابك. للمتابعة، جاوب: كم ناتج {a} + {b}؟"
    if update.callback_query:
...
async def handle_security_captcha(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text.strip()
    expected = context.user_data.get("security_captcha_answer")
    if not text.isdigit() or int(text) != expected:
        await update.message.reply_text("❌ غلط. حاول تاني:")
        return
    context.user_data["awaiting"] = None
    context.user_data.pop("security_captcha_answer", None)
    ...
    await update.message.reply_text("✅ تم التحقق، تقدر تكمل عادي.")
