...
def main_reply_keyboard():
    return ReplyKeyboardMarkup(
        [
            [KeyboardButton("📢 الترويج"), KeyboardButton("🗂️ الأرباح")],
            [KeyboardButton("📱 حسابي"), KeyboardButton("💳 الشيكات")],
            [KeyboardButton("🤖 روبوتاتنا والإحصائيات"), KeyboardButton("🔍 فحص الاشتراك")],
            [KeyboardButton("📋 التعليمات"), KeyboardButton("🔗 روابط مفيدة")],
        ],
...
async def send_welcome(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"👋 أهلاً بك يا {name} في PR GRAM!\n"
        f"📌 لدينا نظام إلزامي للتحقق من الاشتراك في محادثاتك.\n"
        f"يوفر PR GRAM إعدادات مرنة مع مجموعة كبيرة من المرشحات للتحكم في التحقق.\n"
        f"📈 يتيح لك نظام الإعلانات في PR GRAM جذب جمهور حقيقي والترويج لـ:\n"
        f"• 👥 اشتراكات القنوات والمجموعات\n"
        f"• 👁️ مشاهدات المحتوى\n"
        f"• ⚡ شحنات بريميوم\n"
        f"• 🤖 ترويج البوتات\n"
        f"💡 باستخدام البوت فإنك توافق تلقائياً على سياسة الخصوصية الخاصة بنا."
    )
...
async def promo_category_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["draft_task"] = {"category": cat}
    context.user_data["awaiting"] = "task_price"

    mn, sug, mx = db.get_category_price(cat)
    label = db.CATEGORY_LABELS[cat]
    unit_noun = {
        "group": "مشترك واحد/انضمام إلى مجموعة", "channel": "مشترك واحد",
        "bot": "تنفيذ واحد مع البوت", "post": "مشاهدة واحدة", "interaction": "تفاعل واحد",
    }[cat]

    text = (
        f"حدد سعر {unit_noun} لإنشاء مهمة، يجب أن يكون لديك رصيد ليرة كافٍ\n"
        f"🔴 السعر الأدنى للوحدة — {fmt_lira(mn)} ليرة\n"
        f"💡 ملاحظة: الطلب مرتفع حالياً، لذا لضمان الظهور في الصفحات الأولى اجعل السعر لا يقل عن {fmt_lira(sug)} ليرة.\n"
        f"📝 أدخل السعر لـ {unit_noun}:"
    )
    await query.message.reply_text(text, reply_markup=back_kb("promo_menu"))
...
async def process_task_quantity(update, context, qty, is_callback):
    ...
    if total_lira > user["balance"] and not is_callback:
...
async def ask_target_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cat == "bot":
        btn = KeyboardButton("🤖 اختر بوتاً", request_users=KeyboardButtonRequestUsers(request_id=1, user_is_bot=True))
        ...
        btn = KeyboardButton(
            "📢 اختيار قناة/مجموعة",
            request_chat=KeyboardButtonRequestChat(request_id=2, chat_is_channel=(cat == "channel"))
        )
        markup = ReplyKeyboardMarkup([[btn]], resize_keyboard=True, one_time_keyboard=True)
        text = (
...
async def got_shared_chat_or_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if getattr(msg, "chat_shared", None):
        ...
        try:
            ...
            draft["link"] = chat.invite_link or f"https://t.me/{chat.username}" if chat.username else None
        except Exception:
        ...
        if not is_admin_bot:
            await msg.reply_text(
                "⚠️ يجب أن يكون البوت مشرفاً في هذه المحادثة لإطلاق المهمة. أضف البوت كمشرف ثم أعد اختيار المحادثة.",
        ...
        await msg.reply_text(f"✅ تم اختيار: {draft['target_chat_title']}", reply_markup=ReplyKeyboardRemove())
        await msg.reply_text("جاهز للإطلاق:", reply_markup=InlineKeyboardMarkup(keyboard))
        return

    if getattr(msg, "users_shared", None):
        bot_user_id = msg.users_shared.users[0].user_id
        try:
            bot_chat = await telegram_app.bot.get_chat(bot_user_id)
            draft["bot_username"] = bot_chat.username
            draft["link"] = f"https://t.me/{bot_chat.username}"
        except Exception:
            draft["bot_username"] = None
        context.user_data["draft_task"] = draft
        await msg.reply_text(
            f"تم إضافة البوت @{draft.get('bot_username','?')}. اختر الإجراء التالي:",
            reply_markup=ReplyKeyboardRemove()
        )
        keyboard = [
            [InlineKeyboardButton("🚀 نشر المهمة", callback_data="launch_task")],
...
async def handle_post_forward(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin_bot:
        keyboard = [[InlineKeyboardButton("➕ إضافة البوت إلى القناة", url=f"https://t.me/{fwd_chat.username}" if fwd_chat.username else "https://t.me")],
                    [InlineKeyboardButton("🔙 رجوع", callback_data="promo_menu")]]
    ...
    draft["link"] = f"https://t.me/{fwd_chat.username}/{msg.forward_from_message_id}" if fwd_chat.username else None
    context.user_data["draft_task"] = draft
    context.user_data["awaiting"] = None
    await launch_task_now(update, context)
...
async def launch_task_now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await target_msg.reply_text(
        f"✅ تم نشر المهمة № {task_id}.\nتم خصم {cost_text} من رصيدك.\nتابع أخبارنا للمزيد.",
...
async def show_my_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for t in page_tasks:
        ...
        label = f"{emoji} {db.CATEGORY_LABELS.get(t['category'], t['category'])} #{t['task_id']} — {t['current_count']}/{t['target_count']}"
...
async def show_task_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"📌 مهمة #{task_id} — {db.CATEGORY_LABELS.get(t['category'], t['category'])}\n"
    ...
    if cat in ("channel", "group"):
        ...
        for t in tasks[:15]:
            keyboard.append([
                ...
                InlineKeyboardButton(f"↗️ +{fmt_lira(t['unit_price'])} 💰 | اشتراك", url=t.get("link") or "https://t.me")
            ])
...
async def earn_view_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if t["target_chat_id"] and t["link"] and "/" in (t["link"] or ""):
        try:
            msg_no = int(t["link"].rstrip("/").split("/")[-1])
            await telegram_app.bot.forward_message(user_id, t["target_chat_id"], msg_no)
        except Exception:
...
async def earn_open_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.start_completion(task_id, user_id)
    keyboard = [
        [InlineKeyboardButton("➡️ الانتقال", url=t.get("link") or "https://t.me")],
        [InlineKeyboardButton("🛑 تبليغ", callback_data=f"earn_report:{task_id}")],
        [InlineKeyboardButton("🔙 رجوع", callback_data="earn_menu")],
    ]
...
async def earn_rules_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    settings = db.get_settings()
    await query.message.reply_text(settings["rules_text"], reply_markup=back_kb("earn_menu"))
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    task_id = db.get_latest_submitted(user_id)
    if not task_id:
    ...
    keyboard = [
        [InlineKeyboardButton("✅ دفع", callback_data=f"adpay:{task_id}:{user_id}"),
         InlineKeyboardButton("❌ رفض الدفع", callback_data=f"adrejc:{task_id}:{user_id}")],
        [InlineKeyboardButton("♻️ إرسال لمراجعة جديدة", callback_data=f"adrev:{task_id}:{user_id}")],
    ]
    try:
        await telegram_app.bot.send_photo(
            t["owner_id"], photo,
            caption=f"🧾 إثبات جديد — مهمة #{task_id}\nمن: {update.effective_user.username or update.effective_user.full_name}",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
    except Exception:
        pass
...
async def handle_review_note(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await telegram_app.bot.send_message(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("الذهاب مرة أخرى 🔗", url=t.get("link") or "https://t.me")]])
        )
...
async def show_instructions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    settings = db.get_settings()
    text = settings["instructions_text"] or "📋 التعليمات: استخدم «📢 الترويج» لإنشاء مهام، و«🗂️ الأرباح» لكسب الرصيد."
    await update.message.reply_text(text)


async def show_useful_links(update: Update, context: ContextTypes.DEFAULT_TYPE):
    settings = db.get_settings()
    text = settings["useful_links"] or "🔗 لا يوجد روابط مضافة حالياً."
    await update.message.reply_text(text)
...
async def show_checks_placeholder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("💳 قسم الشيكات (سحب الرصيد) قيد التفعيل قريباً.")
...
async def bottom_menu_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    routes = {
        ...
        "💳 الشيكات": lambda: show_checks_placeholder(update, context),
...
telegram_app.add_handler(CallbackQueryHandler(generic_nav_callback, pattern="^(back_main|promo_menu|earn_menu|account_menu|promo_auto_settings)$"))
