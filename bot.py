...
from telegram.ext import (
    ...
    MessageHandler, PreCheckoutQueryHandler, ContextTypes, filters, ApplicationHandlerStop
)
...
async def got_shared_chat_or_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if getattr(msg, "chat_shared", None):
        ...
        if not is_admin_bot:
            await msg.reply_text("جاري التحقق من الصلاحيات...", reply_markup=ReplyKeyboardRemove())
    ...
    if getattr(msg, "users_shared", None):
        ...
        await msg.reply_text(
            f"تم إضافة البوت @{username}. اختر الإجراء التالي:",
            reply_markup=ReplyKeyboardRemove()
        )
        keyboard = [
            [InlineKeyboardButton("🚀 نشر المهمة", callback_data="launch_task")],
...
async def handle_post_forward(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    draft["source_message_id"] = msg.forward_from_message_id
...
async def chq_menu_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("➕ إنشاء شيك", callback_data=f"chq_new:{kind}")],
        [InlineKeyboardButton("🔙 رجوع", callback_data="checks_menu")],
    ]
...
async def chq_new_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    max_amount = user["balance"]  # الحد الأقصى المبدئي (لكل مستخدم بحالة الشخصي)
    ...
    text = (
        ...
        f"الحد الأقصى: {fmt_lira(max_amount)}\n"
        f"الحد الأدنى: {fmt_lira(settings['cheque_min_amount'])}\n"
        f"أدخل مبلغ الشيك:"
    )
...
async def handle_cheque_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text.strip().replace(",", "")
    settings = db.get_settings()
    if not text.isdigit() or int(text) < settings["cheque_min_amount"]:
        await update.message.reply_text(f"❌ المبلغ يجب أن يكون {fmt_lira(settings['cheque_min_amount'])} على الأقل.")
        return
    amount = int(text)
...
async def show_cheque_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    draft = context.user_data.get("cheque_draft", {})
    settings = db.get_settings()
    commission = settings["cheque_commission_percent"]
    total = math.ceil(draft["amount"] * draft.get("max_uses", 1) * (1 + commission / 100))
...
async def handle_cheque_send_user(update: Update, context: ContextTypes.DEFAULT_TYPE, shared_user):
    ...
    if not cheque_id:
        await update.message.reply_text("⚠️ انتهت محاولة إرسال الشيك.", reply_markup=ReplyKeyboardRemove())
        return
    ch = db.get_cheque(cheque_id)
    if not ch or ch["creator_id"] != update.effective_user.id:
        await update.message.reply_text("❌ هذا الشيك غير متاح لك.", reply_markup=ReplyKeyboardRemove())
        return
    me = await telegram_app.bot.get_me()
    link = f"https://t.me/{me.username}?start=cheque_{ch['code']}"
    ...
    try:
        ...
        await update.message.reply_text("✅ تم إرسال الشيك للمستخدم.", reply_markup=ReplyKeyboardRemove())
    except Exception:
        await update.message.reply_text(
...
async def chq_reqchannel_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        "🔒 أرسل معرّف القناة/المجموعة التي يجب الاشتراك بها لتفعيل الشيك (مثال: @my_channel)، أو أرسل - لإلغاء الشرط:"
    )
...
async def handle_cheque_reqchannel(update: Update, context: ContextTypes.DEFAULT_TYPE, cheque_id):
    ...
    context.user_data["awaiting"] = None
    await update.message.reply_text("✔ تم تحديث شرط الاشتراك." if val else "✔ تم إلغاء شرط الاشتراك.")
